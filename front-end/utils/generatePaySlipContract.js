...
const generatePdfDoc = (payslipData, payslipNumber, isFullMonthRange) => {
  ...
  // Determine safe first-period earnings
  const ep1 = toNumber(safePayslipData.grossIncome?.earnPeriod);
  const rate1 = toNumber(safePayslipData.grossIncome?.rate);
  const month1 = toNumber(safePayslipData.grossIncome?.monthlySalary);
  const earnPeriod1 =
    ep1 ?? rate1 ?? (month1 != null ? month1 / 2 : 0);

  const bodyRows = [
    ["", "", "", "", ""], // row 1 blank
    [
      "Rate Per Month",
      formatCurrency(safePayslipData.grossIncome.monthlySalary),
      "",
      "",
      "",
    ],
  ...
  const netPay_cutoff_1 = earnPeriod1 - totalDeductions_cutoff_1;
  ...
  if (isFullMonthRange) {
    ...
    // Determine safe second-period earnings
    const ep2 = toNumber(payslipData.secondPeriodEarnedForPeriod);
    const month2 = toNumber(safePayslipData.grossIncome?.monthlySalary);
    const earnPeriod2 = ep2 ?? (month2 != null ? month2 / 2 : 0);

    const secondTableBodyRows = [
      [
        "Earn for the period",
        formatCurrency(earnPeriod2),
        "",
        "",
        "",
      ],
    ...
    const netPay_cutoff_2 = earnPeriod2 - totalDeductions_cutoff_2;
