...
router.post('/bulk-upload', verifyToken, (req,res,next)=>upload(req,res,(err)=>{ if (err) return res.status(400).json({success:false,message:err.message}); next(); }), bulkUploadSuspensions);
